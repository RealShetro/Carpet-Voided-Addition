...
public class HUDController
{
    ...
    //Bjoel2 memory logger
    private static void log_memory(MinecraftServer server)
    {
        ...
        long usedMemMB = Math.max(TotalMem - FreeMem, 0) / bytesPerMB;
        long allocatedMemMB = TotalMem / bytesPerMB;
        long maxMemMB = MaxMem != Long.MAX_VALUE ? MaxMem / bytesPerMB : -1;
        ITextComponent [] text =  new ITextComponent[]{
                Messenger.m(null, String.format("g %dM / %dM | %dM", usedMemMB, allocatedMemMB, maxMemMB)),
        };
        LoggerRegistry.getLogger("memory").log(() -> text, "UsedMemMB", usedMemMB, "TotalMemMB", allocatedMemMB, "MaxMemMB", maxMemMB);
    }
    //Bjoel2 end
