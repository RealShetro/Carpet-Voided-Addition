...
public class CommandScoreboardPublic extends CommandScoreboard {
    ...
    public String getUsage(ICommandSender sender) {
        return "/scoreboardPublic (joinTeam [teamName])/objectives <list|add|remove|setdisplay> [objective]";
    }
    ...
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos targetPos) {
        ...
        if (args.length == 1) {
            return getListOfStringsMatchingLastWord(args, new String[]{"joinTeam", "objectives"});
        } else {
            if ("objectives".equalsIgnoreCase(args[0])) {
                if (args.length == 2) {
